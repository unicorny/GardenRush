...
bool SwitchButton::switchState(SwitchButtonState state)
{
	...
	switch (state) {
	case SWITCH_BUTTON_DEFAULT: newTexture = mButtonCfg->defaultImg.data(); break;
	case SWITCH_BUTTON_DISABLED: newTexture = mButtonCfg->disabledImg.data(); break;
	case SWITCH_BUTTON_PRESSED: newTexture = mButtonCfg->pressedImg.data(); break;
